...
export function CustomMenuWizard() {
  ...
  const [visitedCategories, setVisitedCategories] = useState<string[]>([categories[0]]); // Mark first as visited
  ...
  const [summaryViewMode, setSummaryViewMode] = useState<'per-person' | 'total'>('per-person'); // Step 3 summary view mode
  // Initialize all categories as collapsed by default
  const [collapsedCategories, setCollapsedCategories] = useState<Record<string, boolean>>(() => {
    const initialCollapsed: Record<string, boolean> = {};
    ['Apéro', 'Starters', 'Intermediate Course', 'Main Courses Meat/Fish', 'Main Courses Veggie', 'Main Courses Vegan', 'Desserts', 'Beverages', 'Technology', 'Decoration', 'Furniture', 'Miscellaneous'].forEach(cat => {
  ...
  const isCategoryLocked = (category: string) => {
    ...
    const maxVisitedIndex = Math.max(...visitedCategories.map(c => categories.indexOf(c)));
  ...
  const handleMainCourseLinkedLogic = (addedItem: MenuItem, addedQuantity: number) => {
    // Only apply logic for Veggie/Vegan main courses
    if (addedItem.category !== 'Main Courses Veggie' && addedItem.category !== 'Main Courses Vegan') {
      return;
    }
    ...
    const meatFishItems = selectedItems
      ...
      .filter(item => item && item.category === 'Main Courses Meat/Fish') as MenuItem[];
    ...
    const reductionNeeded = Math.min(veggieVeganQuantity, totalMeatFishQuantity);
    ...
    if (reductionNeeded > 0) {
      ...
      for (let i = meatFishItems.length - 1; i >= 0 && remainingReduction > 0; i--) {
  ...
  // Get per-person subtotal for all per-person items
  const getPerPersonSubtotal = () => {
    return selectedItems.reduce((total, itemId) => {
      const item = menuItems.find(i => i.id === itemId);
      if (!item || item.pricingType !== 'per-person') return total;
      return total + getItemPerPersonPrice(item);
    }, 0);
  };

  // Get flat-rate subtotal for all flat-rate items
  const getFlatRateSubtotal = () => {
    return selectedItems.reduce((total, itemId) => {
      const item = menuItems.find(i => i.id === itemId);
      if (!item || item.pricingType !== 'flat-rate') return total;
      return total + getItemPerPersonPrice(item);
    }, 0);
  };
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <main className="w-full lg:w-[75%] lg:ml-[25%] bg-background p-4 lg:p-8">
          <div className="max-w-7xl mx-auto">
            <div className="bg-card rounded-lg p-4 lg:p-6 border border-border" style={{ borderRadius: 'var(--radius-card)' }}>
              ...
              {currentStep === 2 && (
                <div>
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-muted-foreground" style={{ fontSize: 'var(--text-base)' }}>
                      Browse through all {categories.length} categories step-by-step • {visitedCategories.length} of {categories.length} visited
                    </p>
                  </div>

                  {/* Main Course Reduction Notification Banner */}
                  {mainCourseReductionMessage && (
                    <div className="mb-6 bg-primary/10 border border-primary/30 rounded-lg px-4 py-3 border-l-4 border-primary" style={{ borderRadius: 'var(--radius)' }}>
                  ...
                  {!['Technology', 'Decoration', 'Furniture', 'Miscellaneous'].includes(selectedCategory) && (
                    <div className="mb-4 bg-secondary border-l-4 border-primary flex items-center gap-2 px-3 py-2" style={{ borderRadius: 'var(--radius)' }}>
                      ...
                      <span className="text-secondary-foreground/50 mx-1" style={{ fontSize: 'var(--text-small)' }}>|</span>
                      <span className="text-secondary-foreground/70" style={{ fontSize: 'var(--text-small)' }}>
                        {selectedCategory === 'Beverages' ? 'Some items are per person, others are flat rate' : 'All items apply per person'}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mb-6">
                    <div ref={categoryContainerRef} className="flex overflow-x-auto gap-2 pb-4 -mx-2 px-2 sm:mx-0 sm:px-0 scrollbar-hide">
                      {categories.map((category) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isVisited && !isActive && !isLocked && <Check className="w-3.5 h-3.5 text-primary" />}
                            <span>{category}</span>
                            {categoryItemCount > 0 && (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2">
                      {/* Category Hero Image */}
                      <div className="mb-4 relative h-48 rounded-lg overflow-hidden" style={{ borderRadius: 'var(--radius-card)' }}>
                        <img
                          src={
                      ...
                      {(selectedCategory === 'Main Courses Veggie' || selectedCategory === 'Main Courses Vegan') && (
                        <div className="mb-5 bg-primary/5 border border-primary/20 rounded-lg px-4 py-3" style={{ borderRadius: 'var(--radius)' }}>
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-muted-foreground mt-1" style={{ fontSize: 'var(--text-small)' }}>
                                Selecting {selectedCategory === 'Main Courses Veggie' ? 'vegetarian' : 'vegan'} mains will automatically reduce your Meat/Fish selections to maintain the correct guest count.
                              </p>
                            </div>
                          </div>
                        </div>
                              ...
                              >
                                ...
                                <div className="flex-1 p-4 flex flex-col">
                                  <div className="flex items-start gap-2 mb-2">
                                    {!['Technology', 'Decoration', 'Furniture', 'Miscellaneous'].includes(item.category) && (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {selectedItems.length > 0 && (
                                <span className="px-2 py-0.5 bg-primary text-primary-foreground rounded-full text-xs" style={{ fontSize: 'var(--text-small)', fontWeight: 'var(--font-weight-medium)' }}>
                                  {selectedItems.length}
                                </span>
                              )}
                            </div>
                          ...
                          {selectedItems.length === 0 ? (
                            ...
                            <>
                                    ...
                                    >
                                      ...
                                      <div className="flex items-start justify-between gap-2 mb-1.5">
                                        <div className="flex-1 min-w-0">
                                          ...
                                          {itemComments[itemId] && (
                                            <p className="text-muted-foreground italic mt-0.5 text-xs truncate" style={{ fontSize: 'var(--text-small)' }}>
                                              "{itemComments[itemId]}"
                                            </p>
                                          )}
                                        </div>
                                      ...
                                      {/* Footer Row: Quantity Badge + Price */}
                                      <div className="flex items-center justify-between pt-1.5 border-t border-border">
                                        ...
                                        <p className="text-foreground" style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--font-weight-semibold)' }}>
                                          CHF {getItemPerPersonPrice(item).toFixed(2)}{item.pricingType === 'per-person' ? <span className="text-muted-foreground text-sm">/person</span> : ''}
                                        </p>
                                      </div>
                                    </div>
                                  );
                              ...
                              <div className="border-t border-border pt-4">
                                ...
                                <div className="bg-primary/5 border border-primary/20 rounded-lg p-4" style={{ borderRadius: 'var(--radius)' }}>
                                  ...
                                  <p className="text-muted-foreground mt-2" style={{ fontSize: 'var(--text-small)' }}>
                                    For {eventDetails.guestCount || '0'} {parseInt(eventDetails.guestCount) === 1 ? 'guest' : 'guests'} • See summary for total
                                  </p>
                                </div>
                              </div>
                            </>
                  ...
                  {isLastCategory() && !allCategoriesVisited() && (
                    <div className="mt-5 p-4 bg-primary/10 border border-primary/20 rounded-lg flex items-start gap-3" style={{ borderRadius: 'var(--radius-card)' }}>
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>
                          You've visited {visitedCategories.length} of {categories.length} categories. Use the category buttons above to explore all menu sections before continuing to the summary.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="md:col-span-2 bg-muted/30 border border-border rounded-lg p-5" style={{ borderRadius: 'var(--radius-card)' }}>
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0" style={{ borderRadius: 'var(--radius)' }}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-foreground" style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--font-weight-semibold)' }}>
                              {eventDetails.guestCount || '-'} {parseInt(eventDetails.guestCount) === 1 ? 'guest' : 'guests'}
                            </p>
                          </div>
                          <p className="text-muted-foreground mt-1" style={{ fontSize: 'var(--text-small)' }}>
                      ...
                      {selectedItems.length === 0 ? (
                        ...
                        <div className="space-y-4">
                          {(() => {
                            ...
                            const categories = ['Apéro', 'Starters', 'Intermediate Course', 'Main Courses Meat/Fish', 'Main Courses Veggie', 'Main Courses Vegan', 'Desserts', 'Beverages', 'Technology', 'Decoration', 'Furniture', 'Miscellaneous'];
                            ...
                            return categories.map(category => {
                              ...
                              return (
                                <div key={category} className="bg-card border border-border rounded-lg overflow-hidden" style={{ borderRadius: 'var(--radius)' }}>
                                          ...
                                          >
                                            ...
                                            <div className="flex-1 min-w-0">
                                              <div className="flex items-start gap-2">
                                                ...
                                                <h6 className="text-foreground font-medium text-sm flex-1">
                                                  {item.name}
                                                </h6>
                                              </div>
                                              ...
                                              <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
                                                {itemVariants[itemId] && item.variants && (() => {
                                                  const variant = item.variants.find(v => v.id === itemVariants[itemId]);
                                                  return variant ? <span>Variant: {variant.name}</span> : null;
                                                })()}
                                                {itemAddOns[itemId] && itemAddOns[itemId].length > 0 && (
                                                  <span>Add-ons: {itemAddOns[itemId].map(addOnId => {
                                                    const addOn = item.addOns?.find(ao => ao.id === addOnId);
                                            ...
                                            <div className="flex-shrink-0 text-right">
                                              ...
                                              <p className="text-muted-foreground text-xs">
                                                {item.pricingType === 'per-person' ? `${quantity}×` : (quantity > 1 ? `${quantity}×` : 'flat')}
                                              </p>
                                            </div>
                                          </div>
                                        );
                          ...
                          <div className="border-t border-border pt-4 mt-4">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>
                                  {selectedItems.length} {selectedItems.length === 1 ? 'item' : 'items'} selected for {eventDetails.guestCount || '0'} {parseInt(eventDetails.guestCount) === 1 ? 'guest' : 'guests'}
                                </p>
                                <p className="text-muted-foreground text-xs mt-1">
                                  Use toggle below for full breakdown
                                </p>
                              </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-card border border-border rounded-lg p-4" style={{ borderRadius: 'var(--radius)' }}>
                          ...
                          {eventDetails.guestCount && (
                            <p className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>
                              Total: {selectedItems.filter(itemId => {
                                const item = menuItems.find(i => i.id === itemId);
                                return item?.dietaryType === 'non-vegetarian' && item?.pricingType === 'per-person';
                              }).reduce((total, itemId) => total + (itemQuantities[itemId] || 1), 0) * parseInt(eventDetails.guestCount)} portions
                            </p>
                          )}
                        </div>
                        ...
                        {summaryViewMode === 'per-person' ? (
                          ...
                          <div className="space-y-4">
                            <div className="bg-card border border-border rounded-lg p-4" style={{ borderRadius: 'var(--radius)' }}>
                              ...
                              {(() => {
                                const foodCategories = ['Apéro', 'Starters', 'Intermediate Course', 'Main Courses Meat/Fish', 'Main Courses Veggie', 'Main Courses Vegan', 'Desserts'];
                                const breakdown = foodCategories.map(cat => {
                                  const items = selectedItems.filter(itemId => {
                                    const item = menuItems.find(i => i.id === itemId);
                                    return item?.category === cat && item?.pricingType === 'per-person';
                                  });
                                  const subtotal = items.reduce((sum, itemId) => {
                                    const item = menuItems.find(i => i.id === itemId);
                                    return sum + (item ? getItemPerPersonPrice(item) : 0);
                                  }, 0);
                                  return { name: cat, count: items.length, subtotal };
                                }).filter(cat => cat.count > 0);

                                return (
                                  <div key="food-categories" className="space-y-2">
                                    {breakdown.map(cat => (
                                      <div key={cat.name} className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                                        <div>
                                          <span className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>{cat.name}</span>
                                          <span className="text-muted-foreground ml-2" style={{ fontSize: 'var(--text-small)' }}>({cat.count})</span>
                                        </div>
                                        <span className="text-foreground font-medium">CHF {cat.subtotal.toFixed(2)}</span>
                                      </div>
                                    ))}
                                  </div>
                                );
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="bg-card border border-border rounded-lg overflow-hidden" style={{ borderRadius: 'var(--radius)' }}>
                              <div className="bg-primary/5 px-4 py-2 border-b border-border">
                                <p className="text-foreground font-semibold" style={{ fontSize: 'var(--text-base)' }}>🍽️ Food Menu</p>
                              </div>
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <div key="food-categories" className="space-y-1">
                                      {breakdown.map(cat => (
                                        <div key={cat.name} className="flex justify-between items-center py-1.5">
                                          <span className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>{cat.name} ({cat.count})</span>
                                          <span className="text-foreground font-medium">CHF {cat.subtotal.toFixed(2)}</span>
                                        </div>
                                      ))}
                                      <div className="pt-2 mt-2 border-t-2 border-primary flex justify-between">
                                        <span className="text-foreground font-semibold">Subtotal Food:</span>
                                        <span className="text-foreground font-semibold">CHF {(() => {
                                          const foodCategories = ['Apéro', 'Starters', 'Intermediate Course', 'Main Courses Meat/Fish', 'Main Courses Veggie', 'Main Courses Vegan', 'Desserts'];
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="bg-card border border-border rounded-lg overflow-hidden" style={{ borderRadius: 'var(--radius)' }}>
                                  <div className="bg-primary/5 px-4 py-2 border-b border-border">
                                    <p className="text-foreground font-semibold" style={{ fontSize: 'var(--text-base)' }}>🍷️ Beverages</p>
                                    <p className="text-muted-foreground text-xs mt-0.5">Pay by consumption • not included in per-person pricing</p>
                                  </div>
                                    ...
                                    {beverageItems.map(itemId => {
                                      ...
                                      return (
                                        <div key={itemId} className="flex justify-between items-center py-1">
                                          <span className="text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>
                                            {item.name} {item.pricingType === 'flat-rate' ? '(per bottle)' : '(per person)'}
                                          </span>
                                          <span className="text-foreground font-medium">CHF {itemTotal.toFixed(2)}</span>
                                        </div>
                                      );
                                    })}
                                    <div className="pt-2 mt-2 border-t border-border flex justify-between">
                                      <span className="text-foreground font-semibold">Subtotal Beverages:</span>
                                      <span className="text-foreground font-semibold">CHF {beverageSubtotal.toFixed(2)}</span>
                                    </div>
                                  </div>
                                </div>
                              );
                            ...
                            {(() => {
                              const serviceCategories = ['Technology', 'Decoration', 'Furniture', 'Miscellaneous'];
                              const serviceItems = selectedItems.filter(itemId => {
                                const item = menuItems.find(i => i.id === itemId);
                                return item && serviceCategories.includes(item.category);
                              });

                              if (serviceItems.length === 0) return null;

                              const serviceSubtotal = serviceItems.reduce((sum, itemId) => {
                                const item = menuItems.find(i => i.id === itemId);
                                return sum + (item ? getItemTotalPrice(item) : 0);
                              }, 0);
                              ...
                              return (
                                <div className="bg-card border border-border rounded-lg overflow-hidden" style={{ borderRadius: 'var(--radius)' }}>
                                  <div className="bg-primary/5 px-4 py-2 border-b border-border">
                                    <p className="text-foreground font-semibold" style={{ fontSize: 'var(--text-base)' }}>✨ Additional Services</p>
                                  </div>
                                  <div className="p-4 space-y-1">
                                    ...
                                    <div className="pt-2 mt-2 border-t border-border flex justify-between">
                                      <span className="text-foreground font-semibold">Subtotal Services:</span>
                                      <span className="text-foreground font-semibold">CHF {serviceSubtotal.toFixed(2)}</span>
                                    </div>
                                  </div>
                                </div>
                              );
                            ...
                            {/* GRAND TOTAL */}
                            <div className="bg-secondary/10 border-2 border-secondary rounded-lg p-4" style={{ borderRadius: 'var(--radius)' }}>
                              <div className="flex justify-between items-center">
                                <div>
                                  <p className="text-foreground font-semibold" style={{ fontSize: 'var(--text-h4)' }}>
                                    TOTAL ESTIMATE
                                  </p>
                                  {eventDetails.guestCount && parseInt(eventDetails.guestCount) > 0 && (
                                    <p className="text-muted-foreground mt-1" style={{ fontSize: 'var(--text-small)' }}>
              ...
              {/* Navigation Buttons */}
              <div className="sticky bottom-0 bg-card flex items-center justify-between mt-3 pt-3 border-t border-border gap-2 -mx-5 px-5 -mb-5 pb-3 lg:-mx-8 lg:px-8 lg:-mb-8 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-30" style={{ borderRadius: '0 0 var(--radius-card) var(--radius-card)' }}>
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {currentStep === 2 && (
                    <Button
                      ...
                      size="sm"
                    >
                      {isLastCategory() && allCategoriesVisited() ? 'Continue to Summary' : 'Next Category'}
                    </Button>
                  )}
