...
import { ChevronLeft, ChevronRight, Users, Clock, Check, MapPin } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className="min-h-[32px] px-2 h-8 text-xs"
            >
                  ...
                  {dayBookings.slice(0, 3).map(booking => (
                    ...
                    >
                      ...
                      {booking.kitchenPdf?.sentStatus === 'sent' && (
                        <Check className="w-3 h-3 flex-shrink-0 text-emerald-600" strokeWidth={3} />
                      )}
                    </div>
                  ))}
          ...
          {upcomingBookings.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcomingBookings.map(booking => (
                <div
                  key={booking.id}
                  className="p-3 bg-background border border-border rounded-lg hover:bg-accent/50 transition-colors"
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {/* Kitchen PDF Status - Simple Tick */}
                      {booking.kitchenPdf?.sentStatus === 'sent' && (
                        <div
                          className="w-5 h-5 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center flex-shrink-0"
                          title="Kitchen PDF sent"
                        >
                          <Check className="w-3 h-3 text-emerald-600" strokeWidth={3} />
                        </div>
                      )}
                      {booking.kitchenPdf?.sentStatus === 'not_sent' && (
                        <div
                          className="w-5 h-5 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center flex-shrink-0"
                          title="Kitchen PDF not sent"
                        >
                          <Clock className="w-3 h-3 text-gray-500" />
                        </div>
                      )}
                      <span
                        className={`px-2 py-0.5 rounded-md border flex items-center gap-1 text-xs font-medium ${booking.status === 'Confirmed' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                          booking.status === 'Touchbase' ? 'bg-primary/10 text-primary border-primary/20' :
                            booking.status === 'New' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                              booking.status === 'Declined' ? 'bg-red-50 text-red-700 border-red-200' :
                          ...
                          style={{
                            backgroundColor:
                              booking.status === 'Confirmed' ? '#10b981' :
                                booking.status === 'Touchbase' ? '#9DAE91' :
                                  booking.status === 'New' ? '#8b5cf6' :
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-between text-muted-foreground" style={{ fontSize: 'var(--text-small)' }}>
                      ...
                      <Button
                        ...
                        className="min-h-[24px] px-2 text-xs h-6"
                      >
                        View
                      </Button>
                    </div>
